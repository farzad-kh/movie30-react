        ...
        {moviesOrseries?.map((item,i) => (
          <SwiperSlide key={item.id} className="   flex flex-wrap bg-base-100 ">
            <div className=" w-[100%] h-[100%] relative object-cover ">
              ...
              >
                <h3 key={item?.title||item?.name} className="sm:text-[26px] text-[14px] leading-normal   text-slate-50  font-semibold  ">
                  {item?.title || item?.name} {item?.release_date?.slice(0, 4)}
                </h3>
                <div className="flex justify-between">
                  <div key={item?.overview} className="max-w-[60%] sm:text-base text-[13.5px] w-full text-[#e2e8ef] mr-6  text-elips-costum2  overflow-hidden">
                    {item?.overview}
                  </div>
                  <Link key={i}
